...
function problemB() {
  ...
  // promise version
  // ???
  const allStanzas = filenames.map(file => promisifiedReadFile(file).then(st => blue(st)));
  Promise.all(allStanzas)
    .then(() => {
      console.log('done')
    });
};
...
function problemC() {
  ...
  }, Promise.resolve(false)) //Se setea el valor inicial del acumulador(promise)
    .then(stanza => {
      blue(stanza)
      console.log('done');
    });
